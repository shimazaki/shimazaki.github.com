...
ki = zeros(N(idx),N(idx)); X = ki; Y = ki;
...
subplot(2,2,2); plot(x,y,'k.','MarkerSize',.5); 
axis square; title('Data'); 
AX = axis;

subplot(2,2,4); 
pcolor(X,Y,ki); shading flat; %bar3(ki);
axis square; axis xy; title('Optimized 2-d histogram'); 
axis(AX); 
