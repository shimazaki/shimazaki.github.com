...
def FormatResult(result) :
    for i in range(0, len(result)) :
		...
		#tweetTime = GetTweetTime(result[i]["created_at"], result[i]["user"]["utc_offset"]); 
		#result[i]["tweet_time"] = tweetTime
    return result
...
def GetDisplayTime(datetimeStr, utc_offset) :
    ...
    #    datetimeStr = dtTarget.strftime("%Y-%m-%d %H:%M:%S")
        datetimeStr = str(60*24)
        
    #1時間以上
    #elif dtdiff.seconds > 3600 :
    #    datetimeStr = str(Decimal(dtdiff.seconds / 3600).quantize(Decimal('1.'), rounding=ROUND_DOWN))

    #1分以上
    elif dtdiff.seconds > 60 :
        datetimeStr = str(Decimal(dtdiff.seconds / 60).quantize(Decimal('1.'), rounding=ROUND_DOWN))

    #1分以内
    else :
        #datetimeStr = str(dtdiff.seconds)
		datetimeStr = str(1)
...
class MainHandler(webapp.RequestHandler):
    ...
    def ShowMyTimeline(self, client, cookie):
        ...
        #timeline_url = "http://api.twitter.com/1/statuses/followers.json"
...
def GetTweetTime(datetimeStr, utc_offset) :
    dtTarget = datetime.datetime.strptime(datetimeStr, "%a %b %d %H:%M:%S +0000 %Y")
    dtNow = datetime.datetime.now()
